...
if ($operation == 'display') {
    ...
    $psize = 10;
    $sql = "SELECT * FROM " . tablename('slwl_fitment_store_brand'). ' WHERE 1 ' . $condition . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize .',' .$psize;

    $list = pdo_fetchall($sql, $params);
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_fitment_store_brand') . ' WHERE 1 ' . $condition, $params);
    ...
    if ($_W['ispost']) {
        ...
        if ($id) {
            pdo_update('slwl_fitment_store_brand', $data, array('id' => $id));
        } else {
            $data['addtime'] = $_W['slwl']['datetime']['now'];
            pdo_insert('slwl_fitment_store_brand', $data);
            $id = pdo_insertid();
        }
        iajax(0, '保存成功！');
    }
    ...
    $one = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_brand') . ' WHERE 1 ' . $condition, $params);


} elseif ($operation == 'delete') {
    $id = intval($_GPC['id']);

    $rst = pdo_delete('slwl_fitment_store_brand', array('id' => $id));
    if ($rst !== false) {
    ...
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_fitment_store_order_goods') . ' WHERE 1 ' . $condition, $params);
    ...
    $pindex = max(1, intval($_GPC['page']));
    $psize = 10;
    $sql = "SELECT * FROM " . tablename('slwl_fitment_store_goods'). ' WHERE 1 '
        . $condition . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize .',' .$psize;

    $list = pdo_fetchall($sql, $params);
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_fitment_store_goods') . ' WHERE 1 ' . $condition, $params);
    $pager = pagination($total, $pindex, $psize);
    ...
    $sql = 'SELECT * FROM ' . tablename('slwl_fitment_store_category') . ' WHERE `uniacid` = :uniacid ORDER BY `parentid`, `displayorder` DESC';
    ...
    if ($_W['ispost']) {
        ...
        if ($id) {
            pdo_update('slwl_fitment_store_goods', $data, array('id' => $id));
        } else {
            $data['addtime'] = $_W['slwl']['datetime']['now'];
            pdo_insert('slwl_fitment_store_goods', $data);
        ...
        $check_one_spec = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_spec') . ' WHERE 1 '
            . $check_condition_spec, $check_params_spec);

        if ($check_one_spec) {
            pdo_update('slwl_fitment_store_spec', $data_spec, array('goodsid' => $id));
        } else {
            $data_spec['addtime'] = $_W['slwl']['datetime']['now'];
            pdo_insert('slwl_fitment_store_spec', $data_spec);
        ...
        if ($_GPC['specop']) {
            ...
            pdo_query("DELETE FROM " . tablename('slwl_fitment_store_goods_option') . " WHERE goodsid=$id");

            foreach ($data_option as $k => $v) {
                pdo_insert('slwl_fitment_store_goods_option', $v);
            }
    ...
    $condition = " AND uniacid=:uniacid AND id=:id ";
    $params = array(':uniacid' => $_W['uniacid'], ':id' => $id);
    $one = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_goods') . ' WHERE 1 ' . $condition, $params);
    $piclist = array();
    if ($one) {
    ...
    // 处理，多规格
    $one_spec = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_spec') . " WHERE goodsid=:goodsid and uniacid=:uniacid", array(":goodsid" => $id, ":uniacid" => $_W['uniacid']));
    if ($one_spec['content']) {
        $spec_items = json_decode($one_spec['content'], true);
    }

    // 处理，规格项目表
    $one_option = pdo_fetchall("SELECT * FROM " . tablename('slwl_fitment_store_goods_option') . " WHERE goodsid=:goodsid ORDER BY id ASC ", array(":goodsid" => $id));
    ...
    if (in_array($type, array('new', 'hot', 'recommand', 'discount'))) {
        $data = ($data==1?'0':'1');
        pdo_update("slwl_fitment_store_goods", array("is" . $type => $data), array("id" => $id, "uniacid" => $_W['uniacid']));
        die(json_encode(array("result" => 0, "data" => $data)));
    }
    if (in_array($type, array('status'))) {
        $data = ($data==1?'0':'1');
        pdo_update("slwl_fitment_store_goods", array($type => $data), array("id" => $id, "uniacid" => $_W['uniacid']));
        die(json_encode(array("result" => 0, "data" => $data)));
    }
    ...
    $rst_1 = pdo_delete('slwl_fitment_store_goods', array('id' => $id));
    $rst_2 = pdo_delete('slwl_fitment_store_goods_option', array('goodid' => $id));
    if ($rst !== false && $rst_2 !== false) {
        iajax(0, $_W['slwl']['lang']['tips_success']);
    } else {
        iajax(1, $_W['slwl']['lang']['tips_not_exist_or_deleted']);
    }
