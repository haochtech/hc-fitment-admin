...
if ($operation == 'display') {
	...
	$sql = "SELECT * FROM " . tablename('slwl_fitment_adsp'). ' WHERE 1 '
	...
	if ($_W['ispost']) {
		$data = array(
			...
			'thumb' => $_GPC['thumb']);
		if ($id) {
			pdo_update('slwl_fitment_adsp', $data, array('id' => $id));
		} else {
	...
	$one = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_adsp') . " where 1 " . $condition, $params);
...
include $this->template('web/module/adsp');
