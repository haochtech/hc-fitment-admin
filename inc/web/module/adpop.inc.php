...
if ($_W['ispost']) {
	...
	if ($_W['slwl']['set']['adpop_set_settings']) {
		...
		$where['setting_name'] = 'adpop_set_settings';
		pdo_update('slwl_fitment_settings', $data, $where);
	} else {
		$data['uniacid'] = $_W['uniacid'];
		$data['setting_name'] = 'adpop_set_settings';
...
if ($_W['slwl']['set']['adpop_set_settings']) {
	$settings = $_W['slwl']['set']['adpop_set_settings'];
}

include $this->template('web/module/adpop');
