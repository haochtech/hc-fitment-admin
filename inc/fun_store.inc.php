...
// 新版商城-首页
if (!function_exists('store_index_data'))
{
	function store_index_data()
	{
		...
		$adv_list = pdo_fetchall('SELECT * FROM ' . tablename('slwl_fitment_store_adv') . ' WHERE 1 '
		...
		$psize_adsp = 5;
		$sql_adsp = "SELECT * FROM " . tablename('slwl_fitment_store_adsp'). ' WHERE 1 '
		...
		$psize_brand = 4;
		$sql_brand = "SELECT * FROM " . tablename('slwl_fitment_store_brand'). ' WHERE 1 '
		...
		$pindex_brand_0 = 1;
		$psize_brand_0 = 1000;
		$sql_brand_0 = "SELECT * FROM " . tablename('slwl_fitment_store_brand'). ' WHERE 1 '
		...
		$pindex_cate = 1;
		$psize_cate = 1000;
		$sql_cate = "SELECT * FROM " . tablename('slwl_fitment_store_category'). ' WHERE 1 '
			. $condition_cate . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex_cate - 1) * $psize_cate .',' .$psize_cate;
		...
		$new_godos_list_field = ' id,title,thumb,price,original_price ';
		$new_godos_list = pdo_fetchall("SELECT " . $new_godos_list_field . " FROM "
			. tablename('slwl_fitment_store_goods') . " WHERE 1 " . $condition_new_godos
		...
		$ishot_list_field = 'id,brandid,title,thumb,price,original_price,intro';
		$ishot_list = pdo_fetchall("SELECT " . $ishot_list_field . " FROM " . tablename('slwl_fitment_store_goods') . " WHERE 1 "
...
// 新版商城-商品详情
if (!function_exists('store_good_detail'))
{
	function store_good_detail()
	{
		...
		$one = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_goods') . ' WHERE 1 ' . $condition_goods, $params_goods);
		if (empty($one)) {
		...
		//浏览量 + 1
		pdo_query("UPDATE " . tablename('slwl_fitment_store_goods')
			. " set viewcount=viewcount+1 where id=:id AND uniacid='{$_W['uniacid']}' ", array(":id" => $goods_id));
		...
		$sql_goods_option = "SELECT * FROM " . tablename('slwl_fitment_store_goods_option'). ' WHERE 1 '
		...
		$fav = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_collect') . ' WHERE 1 ' . $condition, $params);
...
// 新版商城-返回所有一级分类
if (!function_exists('store_category_top'))
{
	function store_category_top()
	{
		...
		$list_category = pdo_fetchall('SELECT * FROM ' . tablename('slwl_fitment_store_category') . ' WHERE 1 '
...
// 新版商城-返回指定分类下的子分类
if (!function_exists('store_category_sub'))
{
	function store_category_sub()
	{
		...
		$one_cate = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_category') . " WHERE 1 "
		...
		if ($one_cate) {
			...
			$children = pdo_fetchall("SELECT * FROM " . tablename('slwl_fitment_store_category') . " WHERE 1 "
...
// 新版商城-获取指定分类下的商品
if (!function_exists('store_goods_list'))
{
	function store_goods_list()
	{
		...
		$psize_goods = 10;
		$sql_goods = "SELECT * FROM " . tablename('slwl_fitment_store_goods'). ' WHERE 1 '
...
if (!function_exists('store_search'))
{
	function store_search()
	{
		...
		// $pindex_brand = 1;
		// $psize_brand = 1000;
		// $sql_brand = "SELECT * FROM " . tablename('slwl_fitment_store_brand'). ' WHERE 1 ' . $condition_brand . " ORDER BY displayorder DESC, id ASC LIMIT " . ($pindex_brand - 1) * $psize_brand .',' .$psize_brand;
		// $list_brand = pdo_fetchall($sql_brand, $params_brand);
		...
		$sql_goods = "SELECT " . $sql_goods_field . " FROM " . tablename('slwl_fitment_store_goods').' WHERE 1 '
...
// 新版商城-收藏
if (!function_exists('store_collect'))
{
	function store_collect()
	{
		...
		if ($operation == 'display') {
			...
			$psize_collect = 1000;
			$sql_collect = "SELECT goodsid FROM " . tablename('slwl_fitment_store_collect'). ' WHERE 1 '
			...
			$is_collect = $data['iscollect'] = $_GPC['iscollect'] == 'true' ? 1 : 0; // 1=收藏，0=取消收藏
			...
			if ($is_collect == 1) {
				...
				$one_goods = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_collect') . ' WHERE 1 '
				...
				if (empty($one_goods)) {
					...
					$rst = pdo_insert('slwl_fitment_store_collect', $data);
					// $id = pdo_insertid();
				...
				$rst = pdo_delete('slwl_fitment_store_collect', array('id' => $one_goods['id'], "uniacid" => $_W['uniacid']));
...
if (!function_exists('store_address'))
{
	function store_address()
	{
		...
		if ($operation == 'display') {
			...
			$psize_address = 10;
			$sql_address = "SELECT * FROM " . tablename('slwl_fitment_store_address'). ' WHERE 1 '
			...
			if ($id) {
				...
				if (isset($_GPC['isdefault'])) {
					...
					pdo_update('slwl_fitment_store_address', array('isdefault'=>0), array('uid' => $uid));
				}
				$rst = pdo_update('slwl_fitment_store_address', $data, array('id' => $id));
				if ($rst !== false) {
					return result(0, '成功');
				} else {
					return result(1, $_W['slwl']['lang']['tips_error']);
				...
				$rst = pdo_insert('slwl_fitment_store_address', $data);
				$id = pdo_insertid();
				if ($rst !== false) {
			...
			$rst = pdo_delete('slwl_fitment_store_address', array('id' => $id, "uniacid" => $_W['uniacid']));
			if ($rst !== false) {
				return result(0, '成功');
			} else {
...
// 新版商城-优惠券，列表
if (!function_exists('store_coupon'))
{
	function store_coupon()
	{
		...
		$psize_coupon = 10;
		$sql_coupon = "SELECT * FROM " . tablename('slwl_fitment_store_sale') . ' WHERE 1 '
		...
		if ($list_sale) {
			...
			$list_sale_user = pdo_fetchall("SELECT * FROM " . tablename('slwl_fitment_store_sale_user') . ' WHERE 1 '
...
// 新版商城-我的优惠券，列表
if (!function_exists('store_coupon_my'))
{
	function store_coupon_my()
	{
		...
		if ($operation == 'display') {
			...
			$psize_sale_my = 10;
			$sql_sale_my = "SELECT * FROM " . tablename('slwl_fitment_store_sale_user') . ' WHERE 1 '
			...
			if ($list_sale_my) {
				foreach ($list_sale_my as $k => $v) {
					...
					if ($_W['slwl']['datetime']['timestamp'] > $timeend) {
						if ($coupon_type == '0') {
							pdo_update('slwl_fitment_store_sale_user', array('status' => '2'), array('id' => $v['id']));
			...
			$sale = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_sale_user') . ' WHERE 1 ' . $condition, $params);
			...
			if ($sale) {
				...
				$one_sale = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_sale') . ' WHERE 1 ' . $condition_sale, $params_sale);
				...
				if ($one_sale) {
					if ($one_sale['total'] > 0) {
						...
						$rst = pdo_insert('slwl_fitment_store_sale_user', $data);

						$rst_2 = pdo_query("UPDATE " . tablename('slwl_fitment_store_sale') . " SET total=total-1, receive=receive+1 WHERE id=:id ", array(':id'=>$one_sale['id']));
...
if (!function_exists('store_cart'))
{
	function store_cart()
	{
		...
		if ($operation == 'add') {
			...
			$goods = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_goods') . " WHERE id = :id", array(':id' => $goodsid));
			if (empty($goods)) {
			...
			$sql_goods_option = "SELECT * FROM " . tablename('slwl_fitment_store_goods_option'). ' WHERE 1 ' . $condition_goods_option;
			...
			$one = pdo_fetch('SELECT id, count FROM ' . tablename('slwl_fitment_store_cart') . ' WHERE 1 ' . $condition, $params);
			...
			if (!$one) {
				...
				$rst = pdo_insert('slwl_fitment_store_cart', $data);
				$id = pdo_insertid();
				...
				$rst = pdo_update('slwl_fitment_store_cart', $data, array('id' => $one['id']));
				if ($rst !== false) {
					return result(0, 'ok');
				} else {
					return result(1, 'err');
			...
			$rst = pdo_delete('slwl_fitment_store_cart', array('from_user' => $uid, 'uniacid' => $_W['uniacid']));
			if ($rst !== false) {
				return result(0, 'ok');
			} else {
				return result(1, 'err');
			...
			$rst = pdo_delete('slwl_fitment_store_cart', array('from_user' => $uid, 'uniacid' => $_W['uniacid'], 'id' => $id));
			if ($rst !== false) {
				return result(0, 'ok');
			} else {
				return result(1, 'err');
			...
			$rst = pdo_update('slwl_fitment_store_cart', $data, array('id' => $id));
		...
		// 全选与反选
			...
			$type = $_GPC['type'];
			if ($type == 'all') {
				$rst = pdo_update('slwl_fitment_store_cart', array('checked'=>'1'), array('from_user' => $uid));
			} else if ($type == 'clean') {
				$rst = pdo_update('slwl_fitment_store_cart', array('checked'=>'0'), array('from_user' => $uid));
			}
			...
			$psize_cart = 100;
			$sql_cart = "SELECT * FROM " . tablename('slwl_fitment_store_cart') . ' WHERE 1 '
...
if (!function_exists('store_brand'))
{
	function store_brand()
	{
		...
		$psize = 10;
		$sql = "SELECT * FROM " . tablename('slwl_fitment_store_brand'). ' WHERE 1 '
...
// 新版商城-品牌商-只获取一条
if (!function_exists('store_brand_one'))
{
	function store_brand_one()
	{
		...
		$one = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_brand') . ' WHERE 1 '
...
// 新版商城-获取指定品牌商的商品
if (!function_exists('store_brand_one_goods'))
{
	function store_brand_one_goods()
	{
		...
		if ($_GPC['id']) {
			...
			$sql = "SELECT ". $fields . " FROM " . tablename('slwl_fitment_store_goods'). ' WHERE 1 '
				. $condition . " ORDER BY displayorder DESC, id ASC LIMIT " . ($pindex - 1) * $psize . ',' .$psize;
			$list = pdo_fetchall($sql, $params);
...
// 新版商城-订单
if (!function_exists('store_order'))
{
	function store_order()
	{
		...
		if ($op == 'add') {
			...
			$address = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_address') . ' WHERE 1 '
				. $condition_address, $params_address);
			...
			if ($settle_accounts == '0') {
				...
				$sql_goods = "SELECT * FROM " . tablename('slwl_fitment_store_goods'). ' WHERE 1 ' . $condition_goods;
				$list_goods = pdo_fetchall($sql_goods, $params_goods);
				// dump(pdo_debug());
				...
				if ($list_goods) {
					...
					if ($goods_items[0]['optionsID'] > 0) {
						...
						if ($one_goods_option['inventory'] <= 0) {
							return result(1, '库存不足');
						}

						foreach ($list_goods as $k => $v) {
							$list_goods[$k]['price'] = $one_goods_option['price'];
							$list_goods[$k]['price_format'] = number_format($one_goods_option['price'] / 100, 2);
					...
					foreach ($list_goods as $k => $v) {
						...
						if ($v['spec_status'] == '1') {
							$rst_count_decrease = pdo_query("UPDATE " . tablename('slwl_fitment_store_goods_option')
								. " SET inventory=inventory-1 WHERE id=:id ", array(':id'=>$goods_items[0]['optionsID']));
							if ($rst_count_decrease === false) {
								sl_ajax(1, '减少库存出错');
							}
						} else {
							$rst_count_decrease = pdo_query("UPDATE " . tablename('slwl_fitment_store_goods')
				...
				$sql_cart = "SELECT * FROM " . tablename('slwl_fitment_store_cart') . ' WHERE 1 ' . $condition_cart . " ORDER BY id DESC ";
				$list_cart = pdo_fetchall($sql_cart, $params_cart);
				...
				$params_goods = array(':uniacid' => $_W['uniacid']);
				$sql_goods = "SELECT * FROM " . tablename('slwl_fitment_store_goods') . ' WHERE 1 ' . $condition_goods . " ORDER BY id DESC ";
				$list_goods = pdo_fetchall($sql_goods, $params_goods);
				...
				$sql_goods_options = "SELECT * FROM " . tablename('slwl_fitment_store_goods_option') . ' WHERE 1 '
					. $condition_goods_options . " ORDER BY id DESC ";
			...
			if ($coupon_id) {
				...
				$one_coupon_my = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_sale_user') . ' WHERE 1 ' . $condition_coupon_my, $params_coupon_my);

				if ($one_coupon_my) {
					$condition_coupon = " AND uniacid=:uniacid AND id=:id ";
					$params_coupon = array(':uniacid' => $_W['uniacid'], ':id'=>$one_coupon_my['saleid']);
					$one_coupon = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_sale') . ' WHERE 1 ' . $condition_coupon, $params_coupon);
				} else {
			...
			if ($one_coupon) {
				$data_order['coupon'] = json_encode($one_coupon);
			}
			$rst_order = pdo_insert('slwl_fitment_store_order', $data_order);
			...
			$rst_coupon_my = pdo_update('slwl_fitment_store_sale_user', $data_update, array('id' => $one_coupon_my['id']));
			...
			foreach ($goods_all as $k => $v) {
				...
				$rst_order_goods = pdo_insert('slwl_fitment_store_order_goods', $data_order_goods);
			...
			if ($settle_accounts == '1') {
				...
				if ($goods_items) {
					...
					if ($flags) {
						...
						$rst_delete_cart = pdo_delete("slwl_fitment_store_cart", $where_cart_clean);
			...
			$one_order = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_order') . ' WHERE 1 ' . $condition_order, $params_order);
			...
			$rst = pdo_update('slwl_fitment_store_order', $data_update, array('id' => $order_id));
			...
			if ($rst !== false) {
				...
				$set = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_store_printers') . ' WHERE 1 ' . $condition . ' limit 1', $params);

				if ($set && $set['enabled']=='1') {
					$printers = json_decode($set['printer_value'], true);
					$content = getOrderInfo($one_order);
			...
			$psize_order = 10;
			$sql_order = "SELECT * FROM " . tablename('slwl_fitment_store_order'). ' WHERE 1 ' . $condition_order . " ORDER BY id DESC LIMIT " . ($pindex_order - 1) * $psize_order .',' .$psize_order;
...
if (!function_exists('store_pay'))
{
	function store_pay($uid, $order, $return_type=true)
	{
		...
		if (empty($uid)) {
			$data_bak = array(
				'code'=>'1',
				'msg'=>$_W['slwl']['lang']['tips_not_exist_user_id'],
			);
			if ($return_type) {
				return $data_bak;
			} else {
				return result(1, 'err', $_W['slwl']['lang']['tips_not_exist_user_id']);
			}
		}
		...
		$condition = " AND id=:id ";
		$params = array(':id'=>$uid);
		$user = pdo_fetch("SELECT * FROM " . tablename('slwl_fitment_users') . ' WHERE 1 ' . $condition, $params);
...
if (!function_exists('getOrderInfo'))
{
	...
	function getOrderInfo($order)
	{
		...
		// $list = pdo_fetch('SELECT * FROM ' . tablename('slwl_fitment_store_order') . ' WHERE 1 ' . $condition_order, $params_order);
