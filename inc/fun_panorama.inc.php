...
// 测试
if (!function_exists('panorama_test'))
{
	function panorama_test()
	{
		global $_GPC, $_W;
...
// 全景-列表-配置
if (!function_exists('panorama_list_config'))
{
	function panorama_list_config()
	{
		global $_GPC, $_W;
		...
		if ($_W['slwl']['set']['set_panorama']) {
			$tmp_config = $_W['slwl']['set']['set_panorama'];
		}
...
// 全景-首页
if (!function_exists('panorama_index_data'))
{
	function panorama_index_data()
	{
		global $_GPC, $_W;
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('slwl_fitment_panorama') . ' WHERE 1 '
...
// 全景-搜索
if (!function_exists('panorama_search'))
{
	function panorama_search()
	{
		global $_GPC, $_W;
...
// 全景-one
if (!function_exists('panorama_one'))
{
	function panorama_one()
	{
...
// 收藏
if (!function_exists('panorama_collect'))
{
	function panorama_collect()
	{
		global $_GPC, $_W;
		...
		if ($operation == 'display') {
			...
			if ($list_collect) {
				...
				$psize_pro = 10;
				$sql_pro = "SELECT * FROM " . tablename('slwl_fitment_panorama'). ' WHERE 1 '
			...
			$condition_fav = " AND uniacid=:uniacid AND id_resource=:id_resource AND user=:user AND type_resource='4' ";
			...
			if ($isfav) {
				if ($one_fav) {
					...
					pdo_query("UPDATE " . tablename('slwl_fitment_panorama') . " set fav_count=fav_count+1 WHERE id=:id ", array(':id'=>$id));
